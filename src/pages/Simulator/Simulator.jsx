...
function AdvancedSimulator({T}){
  ...
  const getPrice=(asset)=>mk[asset]?.price||forexData[asset]?.price||stockData[asset]?.price||0;
  // Computed
  ...
  const assetList=useMemo(()=>{
    ...
    const all=[...keys,...fxKeys,...stKeys];
    if(!search)return all;
    const s=search.toUpperCase();
    return all.filter(k=>{
      const label=forexData[k]?k:stockData[k]?(stockData[k].name||''):'';
      return k.toUpperCase().includes(s)||label.toUpperCase().includes(s);
    });
  },[allPairs,search,forexData,stockData]);
  ...
  const openTrade=()=>{const amount=parseFloat(fAmt);if(!amount||amount<=0)return show('❌ Montant invalide');if(amount>st.cash)return show('❌ Cash insuffisant');const eI=parseFloat(fEntry);const price=eI>0?eI:(mk[fAsset]?.price||forexData[fAsset]?.price||stockData[fAsset]?.price);if(!price)return show('⏳ Prix indisponible');const sl=parseFloat(fSL)||0;const tp=parseFloat(fTP)||0;const cat=mk[fAsset]?'crypto':forexData[fAsset]?'forex':'stock';setSt(prev=>({...prev,cash:prev.cash-amount,nextId:prev.nextId+1,trades:[...prev.trades,{id:prev.nextId,asset:fAsset,cat,type:fType,entry:price,amount,sl,tp,time:Date.now()}]}));setFE('');setFAm('');setFSL('');setFTP('');show(`✅ ${fType.toUpperCase()} ${fAsset} @ ${fmt(price)}`)};
      ...
      <div className="sa-layout">
        <div>
          ...
          <div className="sa-card">
            ...
            {marketTab==='crypto'&&(<div className="sa-mkt-grid">{TOP.slice(0,20).map(sym=>{const d=mk[sym];if(!d)return null;const up=d.move>=0;return(<div key={sym} className="sa-mkt-item" onClick={()=>setFA(sym)} style={{cursor:'pointer',border:fAsset===sym?'2px solid var(--zv-green)':'2px solid transparent'}}><div className="sa-mkt-item__name">{sym}<small>/EUR</small></div><div className="sa-mkt-item__price">{fmt(d.price)}€</div><div className={`sa-mkt-item__ch ${up?'sa-mkt-item__ch--up':'sa-mkt-item__ch--dn'}`}>{up?'+':''}{d.move.toFixed(2)}%</div></div>)})}</div>)}
            {marketTab==='forex'&&(<div className="sa-mkt-grid">{Object.entries(forexData).length>0?Object.entries(forexData).map(([pair,d])=>{const up=(d.move||0)>=0;return(<div key={pair} className="sa-mkt-item" onClick={()=>setFA(pair)} style={{cursor:'pointer',border:fAsset===pair?'2px solid var(--zv-green)':'2px solid transparent'}}><div className="sa-mkt-item__name">{pair}</div><div className="sa-mkt-item__price">{typeof d.price==='number'?d.price.toFixed(4):'...'}</div><div className={`sa-mkt-item__ch ${up?'sa-mkt-item__ch--up':'sa-mkt-item__ch--dn'}`}>{up?'+':''}{(d.move||0).toFixed(2)}%</div></div>)}):<div className="ss-empty">Chargement forex...</div>}</div>)}
            {marketTab==='stocks'&&(<div className="sa-mkt-grid">{STOCK_SYMBOLS.map(s=>{const d=stockData[s.s];if(!d)return null;const up=(d.move||0)>=0;return(<div key={s.s} className="sa-mkt-item" onClick={()=>setFA(s.s)} style={{cursor:'pointer',border:fAsset===s.s?'2px solid var(--zv-green)':'2px solid transparent'}}><div className="sa-mkt-item__name">{d.name}<small>{s.s.startsWith('^')?' (Indice)':''}</small></div><div className="sa-mkt-item__price">{d.price.toFixed(2)}{s.s.includes('.PA')?'€':'$'}</div><div className={`sa-mkt-item__ch ${up?'sa-mkt-item__ch--up':'sa-mkt-item__ch--dn'}`}>{up?'+':''}{d.move.toFixed(2)}%</div></div>)})}</div>)}
          </div>

          <div className="sa-grid2">
            <div className="sa-card"><div className="sa-card__t">Allocation <span>{st.trades.length} pos.</span></div>{trC.map(({trade,m})=>{const pct=(m.val/(totalVal||1))*100;const up=m.pct>=0;return(<div key={trade.id} className="ss-alloc"><div className="ss-alloc__top"><div><strong>{trade.asset}</strong><span className={`sa-badge sa-badge--${trade.type}`}>{trade.type.toUpperCase()}</span><span className="ss-alloc__val">{fE(m.val)} <small>({pct.toFixed(1)}%)</small></span></div><span style={{color:up?'var(--zv-green)':'var(--zv-danger)',fontWeight:700,fontSize:'.8rem',fontFamily:'var(--zv-mono)'}}>{fS(m.pct,'%')}</span></div><div className="ss-bar"><div className="ss-bar__fill" style={{width:`${Math.max(pct,.5)}%`}}/></div></div>)})}<div className="ss-alloc"><div className="ss-alloc__top"><span style={{color:'var(--zv-text-muted)',fontSize:'.8rem'}}>CASH</span><span style={{fontWeight:700}}>{fE(st.cash)}</span></div><div className="ss-bar"><div className="ss-bar__fill ss-bar__fill--cash" style={{width:`${(st.cash/(totalVal||1))*100}%`}}/></div></div>{st.trades.length===0&&<div className="ss-empty">📊 Ouvrez un trade</div>}</div>
        ...
        <aside>
          <div className="sa-card"><div className="sa-card__t">Nouveau Trade</div>
            <div className="ss-form"><label>Actif ({assetList.length})</label><input type="text" placeholder="Rechercher crypto, forex, action..." value={search} onChange={e=>setSearch(e.target.value)} style={{marginBottom:6,fontFamily:'inherit'}}/><select value={fAsset} onChange={e=>setFA(e.target.value)}>{assetList.slice(0,200).map(a=>{const isFx=!!forexData[a];const isSt=!!stockData[a];const label=isFx?a:isSt?`${stockData[a].name} (${a})`:a+'/EUR';const price=isFx?forexData[a].price.toFixed(4):isSt?stockData[a].price.toFixed(2):(mk[a]?fmt(mk[a].price)+'€':'');return <option key={a} value={a}>{label} {price?`— ${price}${isFx?'':isSt?(a.includes('.PA')?'€':'$'):''}`:''}</option>})}</select></div>
            ...
            {(mk[fAsset]||forexData[fAsset]||stockData[fAsset])&&<div className="sa-mkinfo"><span>Prix: <strong>{forexData[fAsset]?forexData[fAsset].price.toFixed(4):fmt(getPrice(fAsset))}€</strong></span><span>24h: <strong style={{color:(mk[fAsset]?.move||forexData[fAsset]?.move||stockData[fAsset]?.move||0)>=0?'var(--zv-green)':'var(--zv-danger)'}}>{(mk[fAsset]?.move||forexData[fAsset]?.move||stockData[fAsset]?.move||0)>=0?'+':''}{(mk[fAsset]?.move||forexData[fAsset]?.move||stockData[fAsset]?.move||0).toFixed(2)}%</strong></span></div>}
          ...
          <div className="sa-card" style={{borderTop:'3px solid var(--zv-danger)'}}><div className="sa-card__t">Compte</div>
            ...
            <button className="ss-btn-reset" onClick={()=>{if(window.confirm('Effacer toutes les données ?')){localStorage.removeItem(STORE_ADV);setSt({initCap:10000,cash:10000,trades:[],closedTrades:[],alerts:[],history:[],nextId:1});if(chartInst.current){chartInst.current.destroy();chartInst.current=null}show('🗑️ Reset effectué')}}}>🗑️ Reset</button>
          </div>
        </aside>
      </div>
      <div className={`sim-toast ${tv?'sim-toast--vis':''}`}>{toast}</div>
      ...
      <div className="container">
        <div className="sim-packs">
          ...
          <div className="sim-pack sim-pack--adv">
            ...
            {isUnlocked('simAdvanced')?<button className="sim-pack__btn sim-pack__btn--pro" onClick={()=>setActiveTab('advanced')}>Ouvrir →</button>:(<div className="sim-pack__unlock"><input type="text" value={codeAdv} onChange={e=>setCodeAdv(e.target.value)} placeholder={t('sim.accessCode')} className={errAdv?'sim-pack__input--err':''} onKeyDown={e=>e.key==='Enter'&&tryUnlock('simAdvanced',codeAdv)}/><button onClick={()=>tryUnlock('simAdvanced',codeAdv)}><Lock size={16}/> {t('sim.unlock')}</button></div>)}
