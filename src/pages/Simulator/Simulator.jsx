      ...
      <div className="sa-layout">
        <div>
          ...
          <div className="sa-card">
            ...
            {marketTab==='forex'&&(<div className="sa-mkt-grid">{Object.entries(forexData).length>0?Object.entries(forexData).map(([pair,d])=>{const up=(d.move||0)>=0;return(<div key={pair} className="sa-mkt-item"><div className="sa-mkt-item__name">{pair}</div><div className="sa-mkt-item__price">{typeof d.price==='number'?d.price.toFixed(4):'...'}</div><div className={`sa-mkt-item__ch ${up?'sa-mkt-item__ch--up':'sa-mkt-item__ch--dn'}`}>{up?'+':''}{(d.move||0).toFixed(2)}%</div></div>)}):<div className="ss-empty">Chargement forex...</div>}</div>)}
            {marketTab==='stocks'&&(<div className="sa-mkt-grid">{STOCK_SYMBOLS.map(s=>{const d=stockData[s.s];if(!d)return null;const up=(d.move||0)>=0;return(<div key={s.s} className="sa-mkt-item"><div className="sa-mkt-item__name">{d.name}<small>{s.s.startsWith('^')?' (Indice)':''}</small></div><div className="sa-mkt-item__price">{d.price.toFixed(2)}{s.s.includes('.PA')?'€':'$'}</div><div className={`sa-mkt-item__ch ${up?'sa-mkt-item__ch--up':'sa-mkt-item__ch--dn'}`}>{up?'+':''}{d.move.toFixed(2)}%</div></div>)})}</div>)}
          </div>
